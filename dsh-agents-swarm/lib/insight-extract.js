...
export const EXTRACTION_PROMPT = [
  ...
  "WHERE A BLOCK HAS A \"Transcript\" SECTION, QUOTE FROM THE TRANSCRIPT. Everything above it in that block is the publisher's own description of the video — a blurb written to sell it, often in the third person, and frequently a claim about the video rather than anything said in it. The transcript is what people actually said, on the record, and it is the only text in the block that can be pointed at a moment in the recording. A sentence like \"She is one of the three authors of the report\" is a description; \"we ended up rewriting the whole evaluation\" is testimony. Prefer the testimony.",
  "Quote the description only when the transcript genuinely does not carry the claim. That happens — a figure stated in the show notes and never read aloud is still a fact the source published — and such a claim is worth keeping. It is simply the weaker of the two, and a reader is shown which one it is.",
